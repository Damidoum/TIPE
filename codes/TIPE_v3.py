...
def dansImage(im, i, j):
    """ vérifie qu'un pixel est dans l'image """
...
# %% Première méthode (mauvais résultats)

def proj_graph(im, m, p, horizontale=False):
    """ Modifie la couleur des pixels sur le long d'une droite de coef directeur m et d'ordonnée à l'origine p """
    if horizontale:
        for i in range(0, x):
            color(img, i, p, 255)
        horizontale = False
...
def proj(im, m, p, horizontale=False):
    """ Projection selon la droite de coef directeur m et d'ordonnée à l'origine p"""
    tot = 0
    if horizontale:
        for i in range(0, x):
            tot += im[i, p]
...
def coupe(img, m, pas=50, horizontale=False):
    ...
    a = round(-m * x)
    p = round((-a + x) / pas)
    # met dans une liste les coupes d'une même direction
    for k in range(a, x, p):
        lst.append(proj(img, m, k, horizontale))
    while len(lst) < pas + 15:  # 15 à ajuster pour qu'il n'y ai pas de dépassement du tableau
        lst.append(0)
    return lst
...
# %% Deuxième méthode avec des coordonnées polaires
...
def coupe_polaire(im, theta, s):
    """ Projection selon une droite de paramètre theta, s """
    tot = 0
...
# %% On propose une dernière méthode avec la fonction rotate (meilleure complexité)
import imageio
import numpy as np
...
def radon(image, steps):
    """ Meilleure méthode pour faire le sinogramme """
    projections = []
    dTheta = -180.0 / steps
